 ...
 *
 * Ported from the same rule in druxt/quickstart. This repository is public
 * and GitHub Pages serves it straight from the default branch, so anything
 * committed here is published the moment it merges - a private URL in a
 * comment, a README, or a lockfile is not buried, it is live.
 *
 * The rule matches the shape of a host, not a list of known hostnames, so
 * the list itself never has to name anything private. That also bounds what
 * it can catch: a bare internal project slug with no URL around it has no
 * shape to match, and stays a review question rather than a lint one.
 */
 ...
 *
 * Userinfo has to be stepped over rather than captured, because a git remote
 * usually carries it - `https://oauth2:TOKEN@host/path` - and capturing
 * `oauth2` instead of the host let the whole URL through.
 */
const URL_HOST =
 ...
 * patterns here, the examples there. Nothing else gets an exemption: an
 * opt-out marker anyone could paste would eventually be pasted over a real
 * leak.
 */
const SELF = ['scripts/lint-private-refs.mjs', 'tests/unit/private-refs.test.js']
