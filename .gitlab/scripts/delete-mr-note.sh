#!/usr/bin/env bash
# Delete a GitLab merge request note identified by a hidden HTML-comment
# marker, if one exists. Usage: delete-mr-note.sh <marker>
set -uo pipefail
...
if [ -n "$existing_id" ]; then
  "${curl_retry[@]}" --request DELETE "$notes_url/$existing_id" >/dev/null
  echo "Deleted stale note $existing_id"
fi
