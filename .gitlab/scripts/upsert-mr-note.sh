#!/usr/bin/env bash
# Upsert a GitLab merge request note identified by a hidden HTML-comment
# marker. Usage: upsert-mr-note.sh <marker> <body-file>
set -euo pipefail
...
if [ -n "$existing_id" ]; then
  "${curl_retry[@]}" --request PUT --form "body=<$body_file" "$notes_url/$existing_id" >/dev/null
  echo "Updated note $existing_id"
else
  "${curl_retry[@]}" --request POST --form "body=<$body_file" "$notes_url" >/dev/null
  echo "Created note"
fi
